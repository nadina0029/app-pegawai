...
@section('page-title', 'Ubah Data Gaji')
...
<div class="max-w-4xl mx-auto mt-6">
    <form action="{{ route('salaries.update', $salary->id) }}" method="POST" id="salaryForm">
        ...
        <div class="glass-panel rounded-3xl p-8 md:p-10 border border-white/10 relative overflow-hidden">
            ...
            {{-- Dekorasi Latar (Rose & Pink untuk Edit) --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    {{-- Nama Karyawan --}}
                    <div>
                        <label for="karyawan_id" class="block text-xs font-bold text-rose-400 uppercase tracking-wider mb-2 ml-1">Nama Karyawan</label>
                    ...
                    {{-- Bulan --}}
                    <div>
                        <label for="bulan" class="block text-xs font-bold text-rose-400 uppercase tracking-wider mb-2 ml-1">Periode Gaji</label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i class="fas fa-calendar-alt text-slate-500 group-focus-within:text-rose-400 transition-colors"></i>
                            </div>
                            {{-- Logic untuk convert format m-Y dari DB ke Y-m untuk input month --}}
                            @php
                                $bulanValue = old('bulan');
                                if (!$bulanValue) {
                                    try {
                                        $bulanValue = \Carbon\Carbon::createFromFormat('m-Y', $salary->bulan)->format('Y-m');
                                    } catch (\Exception $e) {
                                        // Fallback jika format di DB berbeda
                                        $bulanValue = \Carbon\Carbon::parse($salary->bulan)->format('Y-m');
                                    }
                                }
                            @endphp
                    ...
                    {{-- Gaji Pokok (Readonly - Format Rupiah) --}}
                    <div>
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" id="gaji_pokok_display" readonly
                                   ...
                                   placeholder="0">
                            {{-- Input Hidden (Value Lama - Cast to int to remove decimals) --}}
                            <input type="hidden" id="gaji_pokok" name="gaji_pokok" value="{{ (int)old('gaji_pokok', $salary->gaji_pokok) }}">
                        </div>
                    </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative group">
                                ...
                                <input type="text" id="tunjangan_display" 
                                       ...
                                       placeholder="0">
                                <input type="hidden" id="tunjangan" name="tunjangan" value="{{ (int)old('tunjangan', $salary->tunjangan) }}">
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="relative group">
                                ...
                                <input type="text" id="potongan_display" 
                                       ...
                                       placeholder="0">
                                <input type="hidden" id="potongan" name="potongan" value="{{ (int)old('potongan', $salary->potongan) }}">
                            </div>
                        </div>
                        ...
                        <div class="mt-auto pt-4 border-t border-white/10">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="total_gaji_display" readonly
                                       ...
                                       placeholder="0">
                                <input type="hidden" id="total_gaji" name="total_gaji" value="{{ (int)old('total_gaji', $salary->total_gaji) }}">
                            </div>
                        </div>
...
{{-- SCRIPT FORMAT RUPIAH & KALKULATOR --}}
@push('scripts')
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // INIT: Isi nilai awal saat halaman edit dibuka
        // Menggunakan nilai dari hidden input yang sudah diisi PHP
        if (gpHidden.value) gpDisplay.value = formatRupiah(gpHidden.value);
        if (tjHidden.value) tjDisplay.value = formatRupiah(tjHidden.value);
        if (ptHidden.value) ptDisplay.value = formatRupiah(ptHidden.value);
